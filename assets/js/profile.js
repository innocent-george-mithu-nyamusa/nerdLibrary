...
$(document).ready((function () {
    if ($(".profile-menu").length) {
        ...
        "" == (e = (e = (e = e.substring(0, -1 == e.indexOf("#") ? e.length : e.indexOf("#"))).substring(0, -1 == e.indexOf("?") ? e.length : e.indexOf("?"))).substr(e.lastIndexOf("/") + 1)) && (e = "index.php"), $(".profile-menu a").each((function () {
    ...
    if ($(".avatar-button").on("click", (function () {
        ...
        var resourceId = $("#userId").val();

        if("follow-pop" == $(this).attr("id")) {

            $(this).hasClass("is-shifted") ? $.ajax({
                url: "includes/UpdateSpecialResources.php",
                method: "post",
                data: {unFollowUser: resourceId},
                dataType: "text",
                failed: function () {
        ...
        if("invite-pop" == $(this).attr("id")) {

            $(this).hasClass("is-shifted") ? $.ajax({
                url: "includes/UpdateSpecialResources.php",
                method: "post",
                data: {friendUser: resourceId},
                dataType: "text",
                failed: function () {
                    console.log("Failed to unfollow user")
                },
        ...
        $(".pop-button, .avatar-button").toggleClass("is-active"), "follow-pop" == $(this).attr("id") ? $(this).hasClass("is-shifted") ? ($(this).removeClass("is-shifted"), iziToast.show({
            ...
            title: "",
            message: `You are not following ${username} anymore`,
            titleColor: "#fff",
            messageColor: "#fff",
            iconColor: "#fff",
            ...
            title: "",
            message: `You are no longer friends with ${username} `,
            titleColor: "#fff",
            messageColor: "#fff",
            iconColor: "#fff",
            backgroundColor: "#7F00FF",
            ...
            icon: "mdi mdi-send",
            title: "",
            message: `Your invitation has been sent to ${username} `,
            titleColor: "#fff",
            messageColor: "#fff",
            iconColor: "#fff",
            backgroundColor: "#7F00FF",
        ...
        })), $("#submit-cover-picture").on("click", (function () {

            const coverImageFormData = new FormData();
            const coverImage = $("#upload-cover-picture")[0].files;

            coverImageFormData.append('cover_image', coverImage[0]);

            $.ajax({
                url: "includes/UpdateCoverImage.php",
                method: "post",
    ...
    if ($("#upload-profile").length) {
        ...
        $("#upload-profile-picture").on("change", (function () {
            ...
            const profileImage = $("#upload-profile-picture")[0].files;
            profileImageFormData.append('profile_image', profileImage[0]);

            $.ajax({
                url: "includes/UpdateProfileImage.php",
                method: "post",
